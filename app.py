...
gdrive_url = (
    "https://drive.google.com/file/d/1op1s7vX9vf9u--mQ_DidFPNqYInZQPZt/view?usp=sharing"
)
...
emb = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
vs = FAISS.load_local(
    output_dir, emb, index_name=faiss_index_name, allow_dangerous_deserialization=True
)
...
def generate_answer(question: str, chunks: List[Dict[str, Any]]) -> str:
    ...
    msg = RAG_PROMPT.format_messages(
        question=question, context=context, citations=citations
    )
    resp = llm(msg)
